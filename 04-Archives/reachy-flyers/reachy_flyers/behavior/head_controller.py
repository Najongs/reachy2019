...
class Head_Controller(object):
    def __init__(
        ...
        self._list_cmd_y, self._list_cmd_z = [], []
    ...
    def track(self, cmd_yz, prev_yz, goal, input_controller):
       ...
       # dt = self._time[-1] - self._time[-2]
        ...
        #cmd_z += np.clip(np.round(-target[0] * Kpz + (target[0] - prev_z)*self.dt*Kdz, 3),-0.01,0.01)
        #cmd_y += np.clip(np.round(-target[1] * Kpy + (target[1] - prev_y)*self.dt*Kdy, 3),-0.01,0.01)
        cmd_z += np.round(-target[0] * Kpz + (target[0] - prev_z)*self.dt*Kdz, 3)
        cmd_y += np.round(-target[1] * Kpy + (target[1] - prev_y)*self.dt*Kdy, 3)
        
        self._list_cmd_y.append(cmd_y)
        self._list_cmd_z.append(cmd_z)
