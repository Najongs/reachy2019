...
**대화는 무료 로컬 LLM**(Ollama + EXAONE, DGX GPU), **새 동작 생성은 로컬 Qwen**, 시뮬 시각 접지·평가는 **Codex**가 담당한다. 인사·FAQ·목 제스처·물체 주시·오프라인 STT 는 **로봇에서 로컬로** 처리해 인터넷이 끊겨도 계속 동작한다.
...
| **04-Archives/** | 축적 데이터 3층. `raw/` 원시(대화 로그·사람 데이터셋·시뮬 런·운영 로그) → `knowledge/digests/` 일별 요약 → `knowledge/KNOWLEDGE.md` + `knowledge.db` 메인 지식. `reference/` 는 참고용 데모 소스 |
...
[Pi: 로봇]  voice_chat.py ── STT ── HTTP ──▶ [DGX] llm_broker.py ─┬─ 대화: Ollama+EXAONE (무료·GPU)
                 │                                                ├─ 동작: Ollama+Qwen (JSON 생성)
                 │                                                └─ 시뮬 접지·평가: Codex (구조화 JSON)
