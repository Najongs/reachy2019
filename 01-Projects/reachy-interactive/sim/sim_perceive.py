...
계획 모델이 참 3D 좌표를 보면 실물로 절대 못 넘어간다. 실물에는 참값이
없기 때문이다. 그래서 이 모듈이 경계다:

  * detect()      참값을 픽셀로 투영하되 **잡음과 미검출**을 섞어 실물 SSD 의
                  거동을 흉내낸다. 모델은 이것만 본다.
  * overlay()     눈 프레임에 검출 박스를 번호로 그린다 - Codex는 "몇 번 박스"
...
def main():
    ...
    ap.add_argument('--out', default=os.path.join(
        HERE, '..', 'sim_data', 'gallery', 'perceive_demo.png'))
    args = ap.parse_args()
