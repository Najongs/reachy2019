...
**팔 굵기는 실측이 아니라 눈대중이다.** 판정이 이 숫자에 그대로 좌우된다 —
`pick_to_tray` 는 반지름 3.0cm 로 보면 안전(0.9cm), 3.5cm 면 위험(-0.1cm),
4.5cm 면 위험(-1.1cm)이다. **전완과 손 굵기를 자로 재서 `--link-radius`
`--hand-radius` 에 넣으면 그때부터 판정이 실제 값이 된다.**
