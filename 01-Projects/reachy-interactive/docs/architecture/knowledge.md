...
원시 데이터는 Pi 에 쌓이고 지식은 DGX 에서 만들어진다. 둘을 4시간마다
한 바퀴 돌린다 — `ops/dgx/knowledge_sync.sh` (cron `0 8,12,16,20`).

```
① 당겨오기   Pi 대화 로그·사람 사진  ──>  DGX raw/
② 지식화     원시 증분 ──> 일별 요약 ──> 메인 지식 (--sync)
             + 즉답 후보·미리합성 목록·게이트 어휘 재생성
③ 돌려주기   quick_notes / stt_corrections / cached_lines  ──>  Pi
④ 반영       재시작 없이 양쪽이 스스로 집어 든다
```

**재시작하지 않는 것이 핵심이다.** 로봇 서비스를 재시작하면 대화가 끊기고,
브로커를 재시작하면 모델을 다시 적재한다(~40초). 그래서 싱크는 파일만
바꾸고, 양쪽의 감시자가 mtime 을 보고 조용히 갈아 끼운다:

| 어디 | 무엇을 | 감시 주기 | 코드 |
|---|---|---|---|
| 로봇(Pi) | 즉답 노트, STT 교정 | 60초 | `voice_chat.reload_knowledge` |
| 브로커(DGX) | 페르소나, 동작 프롬프트 | 5초 | `llm_broker.PromptFile` |

실측 검증: 페르소나를 고치자 브로커가 재시작 없이 `프롬프트 갱신 반영:
persona.txt (4162자)` 로 집어 들었고, 즉답 규칙을 추가하자 로봇이
`지식 갱신 반영(재시작 없이): 즉답 36건` 을 찍었다.

전송은 **내용 해시가 다를 때만** 한다 — mtime 만 바뀌어도 보내면 로봇이
헛되이 재적재한다. 페르소나·동작 프롬프트는 Pi 로 보내지 않는다(브로커가
쓰는 것이라 DGX 파일에서 바로 다시 읽힌다).

```bash
bash ops/dgx/knowledge_sync.sh              # 한 바퀴 (수동)
bash ops/dgx/knowledge_sync.sh --pull-only  # 당겨오기+지식화만
bash ops/dgx/knowledge_sync.sh --quiet      # cron 용
# 기록: 04-Archives/raw/ops-logs/knowledge_sync.log
```

하루 1회 `daily_update.sh` 는 그대로 둔다 — 무거운 일(사람 인식 백필,
데이터셋 추출, 노트 승격 제안, 주간 다이제스트)을 맡고, 4시간 싱크는
가벼운 왕복만 한다.
